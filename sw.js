...
const CACHE_NAME = 'enjoy-gifts-firebase-v1.0.0';
const urlsToCache = [
  './',
  './index.html',
  './database.js',
...
self.addEventListener('periodicsync', event => {
  ...
  if (event.tag === 'firebase-periodic-sync') {
    event.waitUntil(doFirebaseSync());
  }
});
...
async function cleanupOldCaches() {
  ...
  const oldCaches = cacheNames.filter(name => 
    name.startsWith('enjoy-gifts-') && name !== CACHE_NAME
  );
...
if ('periodicSync' in self.registration) {
  console.log('Service Worker: Periodic sync supported');
} else {
