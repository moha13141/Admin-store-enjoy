import { Client, Databases, Account } from 'https://cdn.jsdelivr.net/npm/appwrite@11.0.0/dist/esm/sdk.js';
...
export async function createNewStore(storeName, ownerName) {
    try {
...
export async function loginExistingStore(storeId) {
    try {
...
export async function listDocuments(collectionId, queries = []) {
    ...
    try {
        const response = await databases.listDocuments(DATABASE_ID, collectionId, [...queries, Query.equal('store_id', currentStoreId)]);
...
// Export collection IDs for external use
export { COLLECTION_STORES, COLLECTION_PRODUCTS, COLLECTION_SALES, COLLECTION_CATEGORIES, COLLECTION_EXPENSES, COLLECTION_REVENUES, COLLECTION_DELETED_INVOICES };

// Export Query object for advanced queries
export { Query } from 'https://cdn.jsdelivr.net/npm/appwrite@11.0.0/dist/esm/sdk.js';
